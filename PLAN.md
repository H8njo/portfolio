...
### IA — 3층 구조 (Pass 1) + 리스트-우선 (2026-06-07 갱신)
- **랜딩(/):** 히어로(488배) → **SELECTED WORK 리스트 하나**(featured 항목이 맨 위에서 자동 강조: 배지·액센트 테두리·"▶ 라이브 데모 보기" 미끼) → About 섹션. "전체 작업 보기 →" 링크로 /work.
- **데모 위치 변경:** column-pager 라이브 데모는 랜딩 임베드 X → **/work/[slug] 상세에서 전체 무대로 작동**(frontmatter `demo: column-pager` 게이트). 이유: 채용자는 리스트 훑고 클릭하는 행동 패턴 → 깔끔한 리스트 + 클릭 유도형 featured가 더 효과적, 데모는 클릭 보상으로 몰입도↑.
...
### 디자인 추가 태스크
- [x] **T8 (P1)** — nav/IA — sticky nav(반응형) + 스킵링크 + 랜딩 featured/큐레이션(데이터 연동) + /work 인덱스 + About/Contact 섹션. 빌드·렌더 검증.
- [x] **T9 (P1)** — states — 커스텀 404 + 에러 바운더리 + 따뜻한 빈 상태(연락 CTA). (PDF 진행단계는 T5 보류)
- [x] **T10 (P1)** — a11y/responsive — 푸터 랜드마크, 44px 터치타깃, 반응형 패딩, 모바일 1열 폴백 검증. (스킵링크·focus·reduced-motion은 T1 선반영)
...
- [x] **T3 (P1)** — landing — T8에서 통합 완료(정적 1열 책자 카드 + featured 슬롯). 히어로 count-up은 T6에서.
- [x] **T4 (P1)** — demo — column-pager-demo(2열, dynamic ssr:false, onPagesGenerated 진입 게이트, 모바일 1열) + "남들 vs 내 방법" 토글 + featured 연결. 브라우저 페이지네이션 검증(스크린샷).
