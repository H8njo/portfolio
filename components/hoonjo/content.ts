    ...
    highlights: [
      ...
      },
      {
        ...
        points: [
          ...
          'PO 수차례 공석 — 저작툴 초기 멤버로서 기획·제품 결정까지 대신 맡음',
          '문제 발견 — 초기 검수 화면은 JSON 직접 수정 + 텍스트 폼: 개발·디자인은 단순하나, 검수자가 JSON 구조·작성 규칙을 통째로 암기해야 하는 구조. 검수자가 자주 새로 합류해 매번 설명해야 하는 온보딩 비용 과다',
          '해결 — 검수자(=실사용자)에게 실제 사용 흐름·불편을 직접 청취 후, 마우스만으로 검수·수정·삭제·추가가 되는 GUI 검수 시스템으로 전면 재설계 (JSON·규칙 암기 불필요)',
          '가변 높이 아이템을 청크(30개) 단위로 측정해 다단 페이지 조판, 7종 데코레이터를 discriminated union으로 타입세이프 모델링',
        ],
        results: ['프론트 단독 · 1,682 커밋(95%)', 'PO 공백 시 제품 주도', 'JSON 암기 → 마우스 GUI 검수'],
      },
      {
